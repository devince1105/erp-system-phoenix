...
## 階段 34 — 票據管理〔2026-08-03〕

- `Note` 實體(收/付方向、種類 支票/本票/匯票、對象、金額、發票/到期日、狀態)+ migration `AddNotes`。
- `NotesController`:CRUD(Admin/會計)+ `POST /{id}/clear` 兌現拋轉銀行傳票(收票 借 銀行/貸 應收帳款;付票 借 應付帳款/貸 銀行)並標記已兌現;`POST /{id}/bounce` 退票。
- 前端 `/accounting/notes`:應收/應付分頁、未兌現/7天內到期/逾期未兌現摘要、到期提示、兌現/退票/刪除。角色限 Admin/會計。
